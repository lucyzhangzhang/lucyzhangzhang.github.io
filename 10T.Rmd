...
```{r chr breakdown, echo = F, warnings = F}
chrfeatures
```
...
All samples were compared against the pooled PA samples of both `C3H10T12` and `decitabine` experiments. Pooled PA further reduces possible experiment-specific differential expression of certain genes. As expected though, H33WT had the lease number of DEGs compared to PA, as it should be. TKO had a lot of down-regulated non-gene elements.
```{r DEGs, echo = F, warnings = F}
buttonplot
```
